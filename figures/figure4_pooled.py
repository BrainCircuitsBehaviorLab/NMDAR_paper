...
def _(
    ...
    pooled_dwell_parts = []
    pooled_switch_parts = []
    pooled_transition_parts = []

    for _task_name in task_names:
        _subject_prefix = f"{_task_name}:"

        _dwell = treatment_dwell_dfs[_task_name].copy()
        _dwell["subject"] = _subject_prefix + _dwell["subject"].astype(str)
        pooled_dwell_parts.append(_dwell)

        _switches = treatment_switch_dfs[_task_name].copy()
        _switches["subject"] = _subject_prefix + _switches["subject"].astype(str)
        pooled_switch_parts.append(_switches)

        _transition = transition_plot_dfs[_task_name].copy()
        _transition["subject"] = _subject_prefix + _transition["subject"].astype(str)
        pooled_transition_parts.append(_transition)

    pooled_dwell_df = pd.concat(pooled_dwell_parts, ignore_index=True)
    pooled_switch_df = pd.concat(pooled_switch_parts, ignore_index=True)
    pooled_transition_df = pd.concat(pooled_transition_parts, ignore_index=True)
    return pooled_dwell_df, pooled_switch_df, pooled_transition_df
...
def _(
    ...
    if model_type != "glmhmmt":
        transition_weights_pooled.set_axis_off()
        transition_weights_pooled.text(0.5, 0.5, "Transition", ha="center", va="center",)
        transition_weights_pooled.text(0.5, 0.3, "Weights", ha="center", va="center",)
    else: 
        palette = {
    ...
    transition_weights_pooled.set_title("Pooled")
    transition_weights_pooled.set_xlabel("")
    transition_weights_pooled.set_ylabel("Transition weight")
    transition_weights_pooled.tick_params(axis="x", labelrotation=0)
    transition_weights_pooled.set_ylim(top=20)
    handles, _ = transition_weights_pooled.get_legend_handles_labels()
    ...
    transition_weights_pooled.legend_.remove()
    if not mount_figure:
        transition_weights_pooled.figure.savefig((path_panels / "pooled_glmhmmt_transition_weights").with_suffix(f".{format}"))
    transition_weights_pooled
    return
...
def _(
    ...
    plt.figure(figsize=fig_size(1, 1), constrained_layout=True)
    histogram_transitions_pooled = (
        plt.gca() if not mount_figure else axd["histogram_transitions_pooled"]
    )
    ...
    histogram_transitions_pooled.set_title("Pooled")
    ...
    if not mount_figure:
        histogram_transitions_pooled.figure.savefig(
            (path_panels / "pooled_drug_state_switch_histogram").with_suffix(
                f".{format}"
            )
        )
    histogram_transitions_pooled
...
def _(axd, fig, mount_figure, path_panels):
    if mount_figure:
        ...
        # P(engaged)
        # axd["single_session_saline_2ADC"].set_title("STM")
        # axd["single_session_saline_2AFC"].set_title("EA")
        # axd["single_session_drug_2ADC"].set_title("")
        # axd["single_session_drug_2AFC"].set_title("")
        axd["single_session_saline_2ADC"].set_xlabel("")
        axd["single_session_saline_2AFC"].set_xlabel("")
        axd["single_session_saline_2AFC"].set_ylabel("")
        ...
        axd["histogram_transitions_pooled"].set_title("")
        axd["dwell_time_pooled"].set_title("")
        axd["transition_weights_pooled"].set_title("")
