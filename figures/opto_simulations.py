...
# title = "Optogenetic Simulations"
# description = "Optogenetic perturbation simulations."
# ///
...
def _(pd, summary_df):
    if summary_df.empty:
        ...
        opto_boxplot_df = summary_df.copy()
        opto_boxplot_df["target_label"] = opto_boxplot_df["target_label"].astype(str)
        opto_boxplot_df["polarity_label"] = opto_boxplot_df["polarity_label"].astype(str)
        opto_boxplot_df["opto"] = opto_boxplot_df["opto"].astype(str)
    return (opto_boxplot_df,)


@app.cell
def _(BOXPLOT_STYLE, PLOT_COLORS, mo, opto_boxplot_df, plt, sns):
    if opto_boxplot_df.empty:
        opto_boxplot = mo.md("Run or load the simulation to create the optogenetic boxplots.")
    else:
        fig_opto_boxplot, _axes = plt.subplots(
            1,
            2,
            figsize=(8.0, 3.6),
            sharey=True,
        ...
        for polarity, _axis in zip(("excitation", "inhibition"), _axes, strict=False):
            panel = opto_boxplot_df[opto_boxplot_df["polarity"] == polarity]
            on_color = PLOT_COLORS[polarity]
            palette = {"Opto off": PLOT_COLORS["opto_off"], "Opto on": on_color}
            sns.boxplot(
                data=panel,
                x="target_label",
                y="frac_correct",
            ...
            _axis.set_title("Excitation" if polarity == "excitation" else "Inhibition")
            _axis.set_xlabel("")
            _axis.set_ylabel("Frac. correct")
            _axis.set_ylim(0.0, 1.02)
            _axis.legend(frameon=False, loc="lower right")

        opto_boxplot = fig_opto_boxplot
    opto_boxplot
    return
...
@app.cell
def _(hard_ttype_selector, pd, sweep_df):
    import polars as pl

    def build_curve_df(raw_pl, delay_code=None):
        data = raw_pl
        if delay_code is not None:
    ...
    def build_delta_long(curve):
        if curve.empty:
            return pd.DataFrame(
                columns=["subject", "opto_amp", "trial_group", "delta_accuracy"]
            )
        return curve.melt(
            id_vars=["subject", "opto_amp"],
            value_vars=["delta_center", "delta_sides"],
            var_name="trial_group",
            value_name="delta_accuracy",
        ).assign(
            trial_group=lambda df: df["trial_group"].map(
                {"delta_center": "Center", "delta_sides": "Sides"}
            )
        )

    def build_accuracy_long(curve):
        if curve.empty:
            return pd.DataFrame(
                columns=["subject", "opto_amp", "trial_group", "frac_correct"]
            )
        return curve.melt(
            id_vars=["subject", "opto_amp"],
            value_vars=["frac_center", "frac_sides"],
            var_name="trial_group",
            value_name="frac_correct",
        ).assign(
            trial_group=lambda df: df["trial_group"].map(
                {"frac_center": "Center", "frac_sides": "Sides"}
            )
        )

    if sweep_df.empty:
        sweep_all_curve = pd.DataFrame()
        sweep_hard_curve = pd.DataFrame()
        hard_ttype_label = f"ttype_c={hard_ttype_selector.value}"
    else:
        raw_pl = pl.from_pandas(sweep_df)
        selected_delay = int(hard_ttype_selector.value)
        sweep_all_curve = build_curve_df(raw_pl)
        sweep_hard_curve = build_curve_df(raw_pl, selected_delay)
        hard_labels = (
        ...
        hard_ttype_label = (
            f"ttype_c={hard_labels[0]}" if hard_labels else f"ttype_c={selected_delay}"
        )

    sweep_delta_df = build_delta_long(sweep_all_curve)
    hard_sweep_delta_df = build_delta_long(sweep_hard_curve)
    sweep_accuracy_df = build_accuracy_long(sweep_all_curve)
    return hard_sweep_delta_df, hard_ttype_label, sweep_accuracy_df, sweep_delta_df
...
@app.cell
def _(mo, plt, sns, sweep_accuracy_df, sweep_status):
    if sweep_accuracy_df.empty:
        sweep_accuracy_plot = mo.md("Run or load a magnitude sweep to create the accuracy curves.")
    else:
        fig_sweep_accuracy, sweep_accuracy_axis = plt.subplots(
            figsize=(5.4, 3.6),
            constrained_layout=True,
        )
        sns.lineplot(
            data=sweep_accuracy_df,
            x="opto_amp",
            y="frac_correct",
            hue="trial_group",
            units="subject",
        ...
        sweep_accuracy_axis.axvline(0.0, color="0.5", linestyle="--", linewidth=0.8)
        sweep_accuracy_axis.set_title("All trials")
        sweep_accuracy_axis.set_xlabel("IC opto current")
        sweep_accuracy_axis.set_ylabel("Model accuracy")
        sweep_accuracy_axis.set_ylim(0.0, 1.02)
        sweep_accuracy_axis.legend(frameon=False, title="")
        sweep_accuracy_plot = mo.vstack(
            [
                mo.md(
...
@app.cell
def _(hard_sweep_delta_df, hard_ttype_label, mo, plt, sns, sweep_delta_df):
    if sweep_delta_df.empty:
        sweep_delta_plot = mo.md("Run or load a magnitude sweep to create the delta-accuracy curves.")
    else:
        fig_sweep_delta, _axes = plt.subplots(
            1,
            2,
            figsize=(8.0, 3.8),
            sharex=True,
        ...
        for data, title, _axis in (
            (sweep_delta_df, "All trials", _axes[0]),
            (hard_sweep_delta_df, hard_ttype_label, _axes[1]),
        ):
            if data.empty:
                _axis.set_title(title)
                continue
            sns.lineplot(
                data=data,
                x="opto_amp",
            ...
            _axis.axhline(0.0, color="0.5", linestyle="--", linewidth=0.8)
            _axis.axvline(0.0, color="0.5", linestyle="--", linewidth=0.8)
            _axis.set_title(title)
            _axis.set_xlabel("IC opto current")
            _axis.set_ylabel(r"$\Delta$ model accuracy")
            _axis.legend(frameon=False, title="")
        sweep_delta_plot = fig_sweep_delta
    sweep_delta_plot
    return
...
def _(mo, sweep_df):
    if sweep_df.empty:
        sweep_table = mo.md("No sweep rows loaded.")
    else:
        sweep_table = mo.ui.table(sweep_df, page_size=20)
    sweep_table
    return
