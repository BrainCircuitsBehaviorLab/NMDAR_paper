...
def _():
    ...
    for base in [Path.cwd(), Path(os.environ.get("PWD", "")).expanduser()]:
        for root in [base, *base.parents]:
            src_path = root / "glmhmmt" / "src"
            if (src_path / "glmhmmt" / "__init__.py").exists():
    ...
    import pandas as pd
    import numpy as np
    from glmhmmt.cli.alexis_functions import filter_behavior
    from glmhmmt.runtime import get_runtime_paths
    from licks import add_lick_data

    paths = get_runtime_paths()
    return Path, add_lick_data, filter_behavior, np, paths, pd, pl
...
def _(combined_df, pl):
    columns_df = pl.DataFrame(combined_df.columns, schema=["Column_Name"])
    combined_df["TrialLag"].value_counts()
    columns_df
    return
...
def _(
    ...
    base_exp_df = (
        ...
        .with_columns(
            ...
            .alias("_session_has_cb")
        )
        .filter(~pl.col("_session_has_cb"))
        .drop("_session_has_cb")
    )
    ...
    base_exp_df = pl.concat(filtered_subjects, how="diagonal")

    base_exp_df_pd = add_lick_data(base_exp_df.to_pandas())
    base_exp_df = pl.from_pandas(base_exp_df_pd)

    base_exp_df = base_exp_df.select(
        [c for c in ["subject", "Trial", "Side", "Choice", "Hit", "Punish", "Session", "ILD", "Filename", "Experiment", "Task", "P" ,"p", "Condition", "AW", "WarmUp", "Date", "CB", "ILI", "nLicks", "RT", "RT2" ]
...
def _(
    ...
    drug_df = pl.concat(filtered_subjects_drug, how="diagonal")

    drug_df_pd = add_lick_data(drug_df.to_pandas())
    drug_df = pl.from_pandas(drug_df_pd)

    drug_df = drug_df.select(
        [c for c in ["subject", "Trial", "Side", "Drug", "Choice", "Hit", "Punish", "Session", "ILD", "Filename", "Experiment", "Task", "P" ,"p", "Condition", "AW", "WarmUp", "Date", "CB", "ILI", "nLicks", "RT", "RT2" ]
